...
public class WorkerThread extends Thread {
	...
	public WorkerThread(Config config, FileSystemManager fileSystemManager, TimeUnit pollSleepUnit, int threadIndex) {
		this.config = config;
	...
	private void process(FileStatus srcFileStatus) throws IOException, InterruptedException {
		...
		try {
			...
			String batchId = srcFile.toString().substring(srcFile.toString().lastIndexOf(filenameBatchidDelimiter) + 1,
			...
			if (config.getCodec() == null && srcFileSize != destFileSize) {
				throw new IOException("event#File sizes don't match, source = " + srcFileSize + ", dest = " + destFileSize);
			}

			log.info("event#Local file size = " + srcFileSize + ", HDFS file size = " + destFileSize+"$batchId#"+batchId);
			...
			log.info("event#Moving staging file '" + stagingFile + "' to destination '" + destFile + "'"+"$batchId#"+batchId);
	...
	private Path stageSource(FileStatus srcFile) throws IOException {
		String filenameBatchidDelimiter = config.getFileNameBatchIdDelimiter();
		Path p = new Path(ScriptExecutor.getStdOutFromScript(config.getWorkScript(), srcFile.getPath().toString(), 60,
		...
		if (p.toUri().getScheme() == null) {
			throw new IOException("event#Work path from script must be a URI with a scheme: '" + p + "'"+"$batchId#"+batchId);
		}
		log.info("event#Staging script returned new file '" + p + " for old " + srcFile.getPath()+"$batchId#"+batchId);
		return p;
	}
	...
	private void verify(Path hdfs, long localFileCRC) throws IOException {
		...
		if (localFileCRC != hdfsCRC) {
			throw new IOException("event#CRC's don't match, local file is " + localFileCRC + " HDFS file is " + hdfsCRC);
		}
	...
	private Path getDestPathFromScript(FileStatus srcFile) throws IOException {
		...
		String batchId = p.toString().substring(p.toString().lastIndexOf(filenameBatchidDelimiter) + 1,
				p.toString().length());
		if (p.toUri().getScheme() == null) {
			throw new IOException("event#Destination path from script must be a URI with a scheme: '" + p + "'"+"$batchId#"+batchId);
		}
		return p;
	}
